...
function [precision, fps] = run_tracker(video, kernel_type, feature_type, show_visualization, show_plots)
    ...
    base_path = 'd:\TrackingPerformance\data\';
    %base_path = 'd:\PETS2009\Crowd_PETS09\performance\';
	%default settings
...
% 		videos(end+1:end+2) = {'Jogging.1', 'Jogging.2'};
		...
		mean_precision = mean(all_precisions(:,20));
        save('mosse_20_5.mat','all_precisions','all_fps');
        
		fps = mean(all_fps);
		fprintf('\nAverage precision (20px):% 1.3f Variace: % 1.3f, Average FPS:% 4.2f\n\n', mean_precision,std(all_precisions(:,20)), fps)
        %compute CI
        SEM_precision = std(all_precisions(:,20))/sqrt(length(all_precisions(:,20)));
        ts = tinv([0.025 0.975], length(all_precisions) - 1); % 95% of confidence intervel
        CI_precisions = mean(all_precisions(:,20)) + ts * SEM_precision
        
        SEM_fps = std(all_fps)/sqrt(length(all_fps));
        ts = tinv([0.025 0.975], length(all_fps) - 1); % 95% of confidence intervel
        CI_fps = mean(all_fps) + ts * SEM_fps
